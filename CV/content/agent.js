/**
 * AGENT AUTONOME SANS ETAT TERMINAL - APPROCHE HEURISTIQUE - 2024 - GPLv3 - MURAT F.
...
//S'il y a des obstacles qui entoure l'agent et seulement quelques voies de sortie, l'agent boucle autour des obstacles sans pouvoir s'en libérer.
const COLORS = {
    AGENT_NORMAL: 'blue',
...
const PATH_COLOR = 'rgba(255, 165, 0, 0.4)'; // Orange semi-transparent
const PATH_WIDTH = 3;
...
class Agent {
    ...
    move() {
        ...
        // Mettre à jour l'état de survie
        this.state.modeSurvival = this.health <= this.config.criticalHealthThreshold * this.config.maxHealth;
        ...
        if (this.state.modeSurvival) {
		 ...
		 if (direction) {
			...
			this.updatePath(); // Ajoutez cette ligne
		}
    ...
    findBestDirection() {
        if (this.state.discoveredRewards.length === 0) {
            // Aucune récompense découverte, utiliser la stratégie précédente
            const directions = [
            ...
            // Construire un segment optimal vers la récompense la plus proche
			const currentPosition = { x: this.x, y: this.y };
			const nearestReward = this.findNearestReward(currentPosition);
			const path = this.findOptimalPath(currentPosition, nearestReward);
			...
			if (path.length > 0) {
				...
				const direction = {
					x: nextStep.x - this.x,
					y: nextStep.y - this.y,
					name: this.getDirectionName(nextStep.x - this.x, nextStep.y - this.y),
				};
				return direction;
            } else {
                // Aucun chemin trouvé, utiliser la stratégie précédente
                const directions = [
                    { x: 0, y: -this.config.stepSize, name: 'Haut' },
                    { x: 0, y: this.config.stepSize, name: 'Bas' },
                    { x: -this.config.stepSize, y: 0, name: 'Gauche' },
    ...
    findNearestReward(currentPosition) {
        let minDistance = Infinity;
        let nearestReward = null;
    
        for (const reward of this.state.discoveredRewards) {
            const distance = Math.abs(reward.x - currentPosition.x) + Math.abs(reward.y - currentPosition.y);
    ...
    findOptimalPath(startPosition, endPosition) {
        ...
        while (queue.length > 0) {
            ...
            const directions = [
                { dx: 0, dy: -this.config.stepSize },
                { dx: 0, dy: this.config.stepSize },
                { dx: -this.config.stepSize, dy: 0 },
                { dx: this.config.stepSize, dy: 0 },
            ];
            ...
            for (const { dx, dy } of directions) {
                ...
                if (newX >= 0 && newX < this.width && newY >= 0 && newY < this.height &&
                    ...
                    if (path.length >= this.config.maxSegmentLength) {
                        return path;
                    }
                }
            }
	...
	updatePath() {
		if (this.state.modeSurvival && this.state.currentPath.length > 0) {
			// Supprimer la première étape du chemin (celle que l'agent vient de franchir)
			this.state.currentPath.shift();
		} else {
			// Effacer le chemin si l'agent n'est pas en mode survie
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const config = {
		animationSpeed: 50, // Temps en millisecondes entre chaque mouvement (plus petit = plus rapide)
        stepSize: 5,
        numObstacles: 2500,
        numRewards: 40,
        gamma: 0.9,
    ...
    speedControl.addEventListener('input', function() {
        config.animationSpeed = this.value;
...
function redrawAll() {
    ...
    if (state.modeSurvival) {
        drawPath(); // Ne dessinez le chemin qu'en mode survie
    }
...
function drawPath() {
    if (state.currentPath.length > 0) {
		ctx.save(); // Sauvegardez l'état du contexte
        ctx.strokeStyle = PATH_COLOR;
        ctx.lineWidth = PATH_WIDTH;
        ctx.beginPath();
        ctx.moveTo(agent.x + config.stepSize / 2, agent.y + config.stepSize / 2);
        state.currentPath.forEach(step => {
            ctx.lineTo(step.x + config.stepSize / 2, step.y + config.stepSize / 2);
        });
        ctx.stroke();
        ctx.restore(); // Restaurez l'état du contexte
    }
}
