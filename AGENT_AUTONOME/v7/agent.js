...
//Peut-être changer la couleur de l'agent lorsqu'il entre en mode survie.
        ...
        for (let i = 0; i < gridWidth; i++) {
            for (let j = 0; j < gridHeight; j++) {
                if (i * stepSize !== reward.x || j * stepSize !== reward.y) {
                    ...
                    const value = Math.pow(gamma, distance / stepSize) * reward.value;
                    stateValues[i][j] = Math.max(stateValues[i][j], value);
                }
            }
        }
    });
...
function updateVisitedPositions(x, y) {
    ...
    function moveAgent() {
		...
		if (agentHealth <= criticalHealthThreshold) {
			...
			agentColor = 'black'; // Changement de couleur en mode survie
		} else {
			// Revenir en mode exploration si la santé n'est plus critique
			modeSurvival = false;
			directionDisplay.textContent = "Exploration";
			agentColor = 'blue'; // Retour à la couleur normale
		}
