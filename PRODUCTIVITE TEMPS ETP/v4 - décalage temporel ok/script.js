...
function calculateProductivity(t, c0, b, m, k, maxTime, timeShift) {
    const adjustedT = Math.max(0, t - timeShift); // Appliquer le décalage temporel
    const competence = c0 + (1 - c0) * (1 - Math.exp(-k * adjustedT / maxTime));
...
function addAgent() {
    ...
    const c0 = parseFloat(document.getElementById('newAgentC0')?.value) || 0;
    ...
    const timeShift = parseInt(document.getElementById('newAgentTimeShift')?.value) || 0; // Nouvel ajout
    const color = getRandomColor();
    
    if (isNaN(c0) || isNaN(b) || isNaN(m) || isNaN(k) || isNaN(maxTime) || isNaN(timeShift)) {
        alert('Veuillez entrer des valeurs numériques valides pour tous les paramètres.');
        return;
    }
    
    agents.push({ name, c0, b, m, k, maxTime, timeShift, color });
    updateAgentsList();
    updateCharts();
}
...
function updateProductivityChart(labels) {
    const datasets = agents.map(agent => ({
        ...
        data: labels.map(t => calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime, agent.timeShift)),
...
function updateEtpChart(labels) {
    const datasets = agents.map(agent => ({
        ...
        data: labels.map(t => calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime, agent.timeShift) / agent.m),
        borderColor: agent.color,
        tension: 0.1
    }));

    const totalEtpData = labels.map(t => {
        return agents.reduce((sum, agent) => sum + calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime) / agent.m, 0);
    });
...
function calculateEquilibriumMonth(agent, maxMonths) {
    for (let t = 0; t <= maxMonths; t++) {
        const productivity = calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime);
