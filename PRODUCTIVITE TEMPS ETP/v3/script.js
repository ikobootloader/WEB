...
function calculateProductivity(t, c0, b, m, k, maxTime) {
...
function addAgent() {
    ...
    if (isNaN(c0) || isNaN(b) || isNaN(m) || isNaN(k) || isNaN(maxTime)) {
        alert('Veuillez entrer des valeurs numériques valides pour tous les paramètres.');
        return;
    }
    
    agents.push({ name, c0, b, m, k, maxTime, color });
    updateAgentsList();
    updateCharts();
}
...
function updateProductivityChart(labels) {
    const datasets = agents.map(agent => ({
        ...
        data: labels.map(t => calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime)),
...
function updateEtpChart(labels) {
    const datasets = agents.map(agent => ({
        ...
        data: labels.map(t => calculateProductivity(t, agent.c0, agent.b, agent.m, agent.k, agent.maxTime) / agent.m),
        borderColor: agent.color,
        tension: 0.1
    }));
